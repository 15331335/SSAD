...
- **Initialestimate**（初始估算）：团队的初步估算，表示与其他故事相比，完成该故事所需的工作量。最小的单位是**故事点（storypoint）**，一般大致相当于一个「理想的人日（man-day）」。
  ...
  💡：**人日计算依赖经验，常用估算人日依据有**页面数（部件数）、报表数和API接口数**；使用真实数据才能正确估算工作量，而模拟数据至少离投产有50%工作量。
...
💡：决定迭代内容时保持轻量，**只安排 1-2 个迭代任务的需求**，并且通过「燃尽表」控制进度。（决定迭代后，backlog 可能需要添加一列 **Iter** 以标识所属迭代）
...
- **[Dinner Now](https://msdn.microsoft.com/zh-cn/library/ff183189.aspx)**
- **Reserve Hotel**
